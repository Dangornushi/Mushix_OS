...
pub fn shutdown() {
    ...
    log!("ACPI Shutdown\n");
    let mut aml = AmlContext::new(Box::new(MushixAmlHandler), DebugVerbosity::None);
    let res = unsafe { AcpiTables::search_for_rsdp_bios(MushixAcpiHandler) };
    match res {
        Ok(acpi) => {
...
#[derive(Clone)]
pub struct MushixAcpiHandler;

impl AcpiHandler for MushixAcpiHandler {
...
struct MushixAmlHandler;

impl Handler for MushixAmlHandler {
