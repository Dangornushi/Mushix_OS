...
//
// mushix Lisp is a lisp-1 like Scheme and Clojure
...
fn default_env() -> Rc<RefCell<Env>> {
    ...
    data.insert(
        ...
        Exp::Func(|args: &[Exp]| -> Result<Exp, Err> {
            ...
            let contents = fs::read_to_string(&path)
                .or(Err(Err::Reason("Could not read file".to_string())))?;
            Ok(Exp::Str(contents))
        }),
    );
    data.insert(
        ...
        Exp::Func(|args: &[Exp]| -> Result<Exp, Err> {
            ...
            let bytes = fs::read(&path, &mut buf)
                .or(Err(Err::Reason("Could not read file".to_string())))?;
            buf.resize(bytes, 0);
        ...
        Exp::Func(|args: &[Exp]| -> Result<Exp, Err> {
            ...
            match &args[0] {
                Exp::List(list) => {
                    ...
                    let s = String::from_utf8(buf).or(Err(Err::Reason(
                        "Could not convert to valid UTF-8 string".to_string(),
                    )))?;
                    Ok(Exp::Str(s))
                }
    ...
    data.insert(
        ...
        Exp::Func(|args: &[Exp]| -> Result<Exp, Err> {
            ...
            match (&args[0], &args[1]) {
                ...
                _ => Err(Err::Reason(
                    "Expected args to be a list and a string".to_string(),
                )),
            }
        }),
    );
    ...
    data.insert(
        ...
        Exp::Func(|args: &[Exp]| -> Result<Exp, Err> {
            ...
            let n = s
                .parse()
                .or(Err(Err::Reason("Could not parse number".to_string())))?;
            Ok(Exp::Num(n))
        }),
    );
    data.insert(
        ...
        match arg {
            ...
            _ => {
                return Err(Err::Reason(
                    "Expected lists of predicate and expression".to_string(),
                ))
            }
        }
    }
...
fn eval_label_args(args: &[Exp], env: &mut Rc<RefCell<Env>>) -> Result<Exp, Err> {
    ...
    match &args[0] {
        ...
        _ => Err(Err::Reason(
            "Expected first argument to be a symbol".to_string(),
        )),
    }
}
...
fn eval_mapcar_args(args: &[Exp], env: &mut Rc<RefCell<Env>>) -> Result<Exp, Err> {
    ensure_length_eq!(args, 2);
    match eval(&args[1], env) {
        Ok(Exp::List(list)) => Ok(Exp::List(
            list.iter()
                .map(|exp| eval(&Exp::List(vec![args[0].clone(), exp.clone()]), env))
                .collect::<Result<Vec<Exp>, Err>>()?,
        )),
        _ => Err(Err::Reason(
            "Expected second argument to be a list".to_string(),
        )),
    }
}
...
fn eval_load_args(args: &[Exp], env: &mut Rc<RefCell<Env>>) -> Result<Exp, Err> {
    ...
    let path = string(&args[0])?;
    let mut code =
        fs::read_to_string(&path).or(Err(Err::Reason("Could not read file".to_string())))?;
    loop {
        ...
        Exp::List(list) => {
            ...
            match eval_built_in_form(first_form, args, env) {
                ...
                None => {
                    ...
                    match first_eval {
                        Exp::Func(func) => func(&eval_args(args, env)?),
                        Exp::Lambda(lambda) => {
