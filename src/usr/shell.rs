...
const AUTOCOMPLETE_COMMANDS: [&str; 36] = [
    ...
    "geotime", "goto", "help", "hex", "host", "http", "httpd", "install", "keyboard", "lisp",
    "list", "memory", "move", "net", "pci", "quit", "read", "shell", "socket", "tcp", "time",
    "user", "vga", "write", "mushipad",
];
...
pub fn prompt_string(success: bool, args: &[&str]) -> String {
    let csi_color = Style::color("Magenta");
    let csi_error = Style::color("Red");
    let csi_reset = Style::reset();
    let pathname = fs::realpath(&args[0]);
    let now_dir = fs::dirname(&pathname);

    format!(
        "{} {}$ {} ",
        now_dir,
        if success { csi_color } else { csi_error },
        csi_reset
    )
}
...
fn cmd_change_dir(args: &[&str], config: &mut Config) -> ExitCode {
    match args.len() {
        ...
        2 => {
            ...
            if args[1] == "~" {
                pathname = fs::realpath("/usr");
            } else {
                pathname = fs::realpath(args[1]);
            }
...
fn exec_with_config(cmd: &str, config: &mut Config) -> ExitCode {
    ...
    loop {
        ...
        if Regex::new("^<=+$").is_match(args[i]) {
            // Redirect input stream
            is_fat_arrow = true;
            left_handle = 0;
        } else if Regex::new("^\\d*=+>$").is_match(args[i]) {
            // Redirect output stream(s)
            is_fat_arrow = true;
            left_handle = 1;
        } else if Regex::new("^\\d*-*>\\d*$").is_match(args[i]) {
            // Pipe output stream(s)
            is_thin_arrow = true;
            left_handle = 1;
            // TODO: right_handle?
        } else {
    ...
    let res = match args[0] {
        ...
        "mushipad" => usr::mushipad::main(&args),
        _ => {
...
fn repl(config: &mut Config, args: &[&str]) -> usr::shell::ExitCode {
    println!();
    ...
    while let Some(cmd) = prompt.input(&prompt_string(success, args)) {
...
fn test_shell() {
    ...
    assert!(api::fs::read_to_string("/test")
        .unwrap()
