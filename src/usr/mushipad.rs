...
impl Editor {
    ...
    fn print_status(&mut self, status: &str, background: &str) {
        let color = Style::color("Green").with_background(background);
    ...
    fn print_editing_status(&mut self) {
        ...
        match self.mode {
            0 => mode_string = "Nomal".to_string(),
            1 => mode_string = "Insert".to_string(),
            _ => mode_string = "Nomal".to_string(),
        }
        let start = format!("{} | Editing '{}'", mode_string, path);
        ...
        self.print_status(&status, "Black");
    }
    ...
    pub fn run(&mut self) -> usr::shell::ExitCode {
        ...
        loop {
            ...
            } else if escape == true && c == 'q' {
                print!("\x1b[2J\x1b[1;1H"); // Clear screen and move cursor to top
                print!("\x1b[?25h"); // Enable cursor
                break;
            } else if escape == true && c == 'i' {
                self.mode = 1;
                escape = false;
                self.print_editing_status();
                for _ in 0..100 {}
                //    print!("\x1b[{};{}H", self.cursor.y + 1, self.cursor.x + 1);
            } else if escape == true && c == 'h' {
